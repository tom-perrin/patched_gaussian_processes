 ...
 $$ \mathbb{E} \left[ f_*^{(k)} \mid y, \delta = 0 \right] = \left( c_{*\mathcal{D}}^{(k)} - c_{*\delta}^{(k)} C_{\delta\delta}^{-1} C_{\delta\mathcal{D}} \right) \left(  C_{\mathcal{D}\mathcal{D}} - C_{\mathcal{D}\delta} C_{\delta\delta}^{-1} C_{\delta\mathcal{D}} \right)^{-1} y.$$
	...
	\end{frame}
  	 ...
  	 \begin{frame}{nearest-Neighbors}
		...
		\item For non-stationary covariance functions, the nearest neighbors can be found by evaluating the covariance between $x^*$ and all observation locations and selecting the $m$ locations with the highest covariance values.
		\end{itemize}
		\begin{center}
