...
\begin{document}
	...
	\begin{frame}{Kriging}
		...
		\begin{itemize}
   			 \item \textbf{Predicted value :} $f(x^*) \approx c_{x*}^T(\sigma^2I+C_{xx})^{-1}y.$
...
\section{The "Big $N$" Problem and Beyond}
 ...
 $$ \mathbb{E} \left[ f_*^{(k)} \mid y, \delta = 0 \right] = \left( c_{*\mathcal{D}}^{(k)} - c_{*\delta}^{(k)} C_{\delta\delta}^{-1} C_{\delta\mathcal{D}} \right) \left(  C_{\mathcal{D}\mathcal{D}} - C_{\mathcal{D}\delta} C_{\delta\delta}^{-1} C_{\delta\mathcal{D}} \right)^{-1} y.$$
	...
	\begin{frame}{Algorithm}
		\textbf{Steps :}
		\begin{enumerate}
			\item Partition the domain : $\Omega = \bigcup_{k=1}^K \Omega_k.$
			\item Create the pseudo-observations $\delta_{k,\ell}$ on each $\Gamma_{k,\ell}.$
			\item Use MLE to choose each covariance function $C_k(\cdot,\cdot).$
			\item Compute $\mathbb{E}\left[f_*^{(k)} \mid y, \delta = 0 \right]$ and $\mathbb{V}\left[f_*^{(k)} \mid y, \delta = 0 \right]$ :
			\begin{itemize}
				\item Comptute $Q = \left(C_{\mathcal{D} \mathcal{D}}-C_{\mathcal{D} \delta} C_{\delta\delta}^{-1} C_{\delta\mathcal{D}} \right)^{-1}.$
				\item Compute Cholesky decomposition of $C_{\delta\delta} = LL^T$
				\item Deduce $v = L^{-1} C_{\delta\mathcal{D}}$ and $w_* = L^{-1} c_{\delta*}^{(k)}.$
			\end{itemize}
			\item Be happy \Changey{2}
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Intuitive Idea}
		...
		\end{itemize}
		$$f_\theta \sim \mathcal{N}(0, K_\theta)$$ \\
		$$f(x^*) = \sum_{j=1}^{n}A_j y_j + \varepsilon $$ 

		\vspace{0.5cm}
		\begin{itemize}
			\item With $A_j = \delta_{N(x^*)}$ Where $N(x^*)$ is the set of $x^*$ nearest neighbors.
			\item Restraining the Kriging to the $m$ nearest neighbors of each prediction location $x^*$ leads to a sub-optimal prediction.
			\item Our Kriging variance is now  $c_{**} - c_{N(x^*)*}^T(C_{N(x^*)N(x^*)})^{-1}c_{N(x^*)*}.$ for the given $x^*$.
		\end{itemize}
	\end{frame}
  	 ...
  	 \begin{frame}{Nearest-Neighbors}
		Nearest-Neighbors are the points that have the highest spatial correlation with the prediction location $x^*$:
		\begin{itemize}
		\item In the case of stationary covariance functions, these are simply the closest points to $x^*$.
		\item For non-stationary covariance functions, the nearest neighbors are the maxima of the covariance function with respect to $x^*$.
		\end{itemize}
		\vspace{-0.5cm}
		\begin{center}
        % Syntaxe : \animategraphics[options]{FPS}{base_nom}{début}{fin}
        \animategraphics[autoplay,loop,width=0.4\textwidth]{0.8}{frames/frame_}{000}{009}
    \end{center}
	...
	\end{frame}
   	...
   	\begin{frame}{Choosing m}
		\begin{itemize}
			\item First idea : running cross-validation for different values of $m$ and choosing the one that minimizes the prediction error.
			\item In Bayesian Inference, m is treated as a parameter and updated in the MCMC algorithm.
		\end{itemize}

	\vspace{0.5cm}
	$$ p(f_\theta, \beta, \theta, \tau^2 , m \mid y) \propto p(y \mid f_\theta, \beta, \tau^2) \times p(f_\theta \mid \theta) \times p(\beta)p(\theta)p(\tau^2)p(m)$$
	\end{frame}
	...
	\begin{itemize}
		\item Recall the complexity $\longrightarrow \boldsymbol{\mathcal{O}(\frac{N^3}{K^2} + d^3B^3K )}$
	...
	For Method II (\textbf{NN-Kriging}), the expense is located in the computation of $$c_{**} - c_{N(x^*)*}^T(C_{N(x^*)N(x^*)})^{-1}c_{N(x^*)*}.$$ 
	
	\vspace{1cm}
  	\textbf{Reminder}
    	\begin{itemize}
      		\item <1-> $N$ observations of a space of dimension $d$
	...
	\pause
 	 \textbf<2->{Complexity}
 	  \begin{itemize}
       		\item<1-> Computing a new location$(C_{N(x^*)N(x^*)})^{-1}\rightarrow \boldsymbol{\mathcal{O}(m^3)}$
	     	\item<2-> Computing n new location$(C_{N(x^*)N(x^*)})^{-1}\rightarrow \boldsymbol{\mathcal{O}(nm^3)}$
...
\begin{frame}{Results II}
	In \cite{datta-2016}, Tests have been made to mesure performances of NNGP
	 \begin{figure}
    		\includegraphics[width=.8\textwidth]{figures/results_NNGP.png}
    		%\vspace*{-0.3cm}
     		\caption{results of multivariate simulations for several values of m}
   	 \end{figure}
